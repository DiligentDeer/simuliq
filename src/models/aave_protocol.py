...
class AaveProtocolDTO(ProtocolDTO):
    ...
    def __post_init__(self):
        ...
        logger.info(f"Initialized Aave Protocol DTO for network: {self.chain.network}")

    def __repr__(self):
        """Return a string representation of the Aave Protocol DTO instance."""
        return f"Aave Protocol DTO(network={self.chain.network})"
    ...
    def get_reserve_list(self, block_number: Optional[int] = None) -> List[Dict[str, Union[str, int]]]:
        ...
        if not self.aave_data_provider_address:
            raise ValueError(f"Aave data provider address is not set for {self.chain.network}")
    ...
    def get_asset_data(self, asset: str, block_number: Optional[int] = None) -> Dict[str, Union[str, int]]:
        ...
        if not self.aave_pool_address:
            raise ValueError(f"Aave pool address is not set for {self.chain.network}")
    ...
    def get_user_balance(self, user_address_list, asset, block_number=None):
        ...
        if not self.batch_data_provider_address:
            raise ValueError(f"Batch data provider address is not set for {self.chain.network}")
    ...
    def get_current_price(self, token_address: str) -> Optional[float]:
        ...
        url = f"{base_url}/{self.chain.network}:{token_address}?searchWidth=12h"
        ...
        data = response.json()
        price_info = data.get('coins', {}).get(f'{self.chain.network}:{token_address}', {})
    ...
    def get_aave_supported_asset_data_to_csv(self) -> None:
        ...
        data_df.to_csv(f'aave_supported_asset_data_{self.chain.network}_{TIME_STAMP}.csv', index=False)
    
    
    def get_users(self) -> pd.DataFrame:
        if self.holder_query_id is None:
            raise ValueError(f"Holder query ID is not set for {self.chain.network}")
    ...
    def get_users_to_csv(self) -> None:
        ...
        data_df.to_csv(f'user_data_{self.chain.network}_{TIME_STAMP}.csv', index=False)
    ...
    def get_user_position_data_to_csv(self) -> None:
        ...
        data_df.to_csv(f'user_position_data_{self.chain.network}_{TIME_STAMP}.csv', index=False)
    ...
    def get_user_position_data_optimized_to_csv(self) -> None:
        ...
        data_df.to_csv(f'user_position_data_optimized_{self.chain.network}_{TIME_STAMP}.csv', index=False)
